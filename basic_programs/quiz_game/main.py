...
from shared.path_utils import get_project_dir, read_json, write_json


PROJECT_DIR = get_project_dir(Path(__file__))
QUESTIONS_FILE = PROJECT_DIR / "questions.json"
HIGHSCORES_FILE = PROJECT_DIR / "highscores.json"
...
def load_questions() -> List[Question]:
    ...
    data = read_json(QUESTIONS_FILE, default)
...
def save_highscore(name: str, score: int, total: int) -> None:
    ...
    data = read_json(HIGHSCORES_FILE, default=[])
    data.append(entry)
    data.sort(key=lambda x: (-x["score"], x["name"]))
    data = data[:10]
    write_json(HIGHSCORES_FILE, data)
